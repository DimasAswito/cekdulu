<div class="max-w-3xl mx-auto py-10 px-4 sm:px-6 lg:px-8">
    ...
    <form wire:submit="save" class="space-y-8 bg-white shadow-sm rounded-xl border border-brand-100 p-6 sm:p-8">
        <div>
            ...
            <div class="grid grid-cols-2 sm:grid-cols-3 gap-3">
                @foreach ($this->allConditions as $condition)
                    <label wire:key="condition-{{ $condition->id }}" class="flex items-center gap-2 rounded-lg border border-gray-200 px-3 py-2 cursor-pointer hover:border-brand-400 has-[:checked]:border-brand-600 has-[:checked]:bg-brand-50">
                        <input type="checkbox" wire:model="conditions" value="{{ $condition->id }}" class="rounded border-gray-300 text-brand-600 focus:ring-brand-500">
        ...
        <div>
            ...
            <div class="grid grid-cols-2 sm:grid-cols-3 gap-3">
                @foreach ($this->allAllergens as $allergen)
                    <label wire:key="allergen-{{ $allergen->id }}" class="flex items-center gap-2 rounded-lg border border-gray-200 px-3 py-2 cursor-pointer hover:border-brand-400 has-[:checked]:border-brand-600 has-[:checked]:bg-brand-50">
                        <input type="checkbox" wire:model="allergens" value="{{ $allergen->id }}" class="rounded border-gray-300 text-brand-600 focus:ring-brand-500">
        ...
        <div>
            ...
            <textarea wire:model="notes" id="notes" rows="3" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-brand-500 focus:ring-brand-500"></textarea>
